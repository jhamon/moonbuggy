...
Pass `--trace-json` and `moonbuggy run <id>` prints the verdict plus the exact
evidence that produced it, as one JSON object, and appends the same line to
`.moonbuggy/traces.jsonl`. The file is append-only by design: the audit log's
value is that yesterday's evidence survives today's verdict, so re-measuring a
mutant adds a line rather than rewriting one. `jq .verdict.status` selects the
same mutants `grep KILLED` does — the status keyword travels inside the JSON
verbatim:
...
```{code-block} json
{
  ...
  "verdict": {
    "status": "KILLED",
    "killreason": "assertion_failed",
    "assert": ["tests/test_pricing.py::test_bulk"],
    "crash": null
  }
}
```

### schema_version: reading a trace safely

`trace_schema` is the version of the trace's own shape — independent of the
`results.jsonl` record's `schema` field, because a trace is a separate document
that moves for separate reasons. **Check it before you parse anything else.**
A reader that sees a `trace_schema` it does not know should refuse the line,
not guess: version 1 freezes the key list above, and a future version may add,
rename, or re-type keys.

What version 1 guarantees:

- **The key set is frozen.** Exactly the keys shown: identity (`id`, `file`,
  `line`, `operator`, `original`, `mutated`), the `selection` object
  (`selected`, `failed`), and the `verdict` object (`status`, `killreason`,
  `assert`, `crash`). No additional keys, none missing.
- **Every verdict names its evidence, and nothing else.** A `KILLED` or
  `KILLED_BY_ERROR` lists the exact tests that objected in `verdict.assert`
  (sorted — the order is part of the contract); a `SURVIVED` names the
  surviving mutation in `original`/`mutated` because no test objected; a
  `NO_COVERAGE` carries the empty selection; a `SUSPICIOUS` names its cause
  via `verdict.killreason` (`flaky_probe` vs `execution_crash`) and, for a
  crash, the string in `verdict.crash`. `TIMEOUT` and `SKIPPED` carry no
  invented evidence — empty assert, null crash, null killreason.
- **`verdict.killreason` uses the frozen killreason vocabulary** — the same
  tokens the JSONL record carries — and `verdict.status` is one of the seven
  plaintext keywords from [Statuses](#statuses).
- **One line, one mutant.** The same one-JSON-object-per-line discipline as
  `results.jsonl`: a reader with one line has everything it needs, and no
  value contains a newline.

The shape is pinned by a golden-file test (`tests/test_trace_golden.py`): the
fixture above is generated by calling the same `Verification.trace()` the CLI
calls, so any key change fails CI before it reaches a consumer.
