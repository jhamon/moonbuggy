...
The gate compares wall-clocks only within one host: `verdict()` checks the
`host` field and re-primes the baseline when it changes, so a Darwin-primed
baseline handed to an ubuntu CI runner re-arms instead of false-failing (the
committed baseline is a priming artifact; CI's first run on its own host
overwrites it before the gate has any teeth).
