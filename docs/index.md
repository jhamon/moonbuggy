...
noticed. That is a gap in your suite, named down to the line. A `NO_COVERAGE`
line is the other kind of gap: no test executes it at all.
...
with no knowledge of the schema (`grep NO_COVERAGE` gets the lines no test
reaches), and the JSONL alongside it is the same data for
